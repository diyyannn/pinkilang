            ...
            <div class="sidebar">
                ...
                <div class="menu-section">
                    ...
                    <a href="/prive" class="menu-item"> Prive</a>
                     ...
                     <a href="/pelunasan-pendapatan-dimuka" class="menu-item"> Pelunasan Pendapatan Diterima Dimuka</a>
    ...
    if request.method == "POST":
        try:
            ...
            if harga_jual_per_ekor <= 0 or jumlah_ekor <= 0:
                ...
                # 🔹 BUAT JURNAL UMUM OTOMATIS - PAKAI 'LAINNYA'
                jurnal_data = [
                    {
                        "user_email": user_email,
                        "tanggal": tanggal,
                        ...
                        "debit": jumlah_dp,
                        "kredit": 0,
                        "keterangan": f"[DP AWAL] DP {dp_persen}% dari {nama_customer}: {keterangan}",
                        "transaksi_type": "LAINNYA",  # DIUBAH
                        "referensi_id": None,
                        "created_at": datetime.now().isoformat()
                    },
                    {
                        ...
                        "debit": 0,
                        "kredit": jumlah_dp,
                        "keterangan": f"[DP AWAL] DP {dp_persen}% penjualan {jenis_ikan} ke {nama_customer}",
                        "transaksi_type": "LAINNYA",  # DIUBAH
                        "referensi_id": None,
                        "created_at": datetime.now().isoformat()
                    }
                ...
                if supabase:
                    ...
                    pdd_id = pdd_result.data[0]['id'] if pdd_result.data else None
                    
                    # Update referensi_id di jurnal
                    if pdd_id:
                        for jurnal in jurnal_data:
                            jurnal['referensi_id'] = pdd_id
                    ...
                    <div class="message success">
                        ...
                        <div style="margin-top: 10px; background: #f0f8ff; padding: 10px; border-radius: 5px; border-left: 4px solid #ff66a3;">
                            ...
                            • 💳 Metode: {metode_pembayaran.title()}<br>
                            • 📝 <em>Jenis: Pendapatan Diterima Dimuka (LAINNYA)</em>
                        </div>
                    </div>
                    '''
                else:
        ...
        <div class="container">
            ...
            <div class="content">
                ...
                <div class="action-buttons">
                    ...
                    <a href="/pelunasan-pendapatan-dimuka" class="btn" style="background: #4CAF50; color: white;">✅ Pelunasan DP</a>
...
# ============================================================
# 🔹 ROUTE: Pelunasan Pendapatan Diterima Dimuka - DIUBAH
# ============================================================
@app.route("/pelunasan-pendapatan-dimuka", methods=["GET", "POST"])
def pelunasan_pendapatan_dimuka():
    if not session.get('logged_in'):
        return redirect('/login')
    ...
    # Ambil data DP yang belum dilunasi
    pdd_belum_lunas = []
    total_dp_aktif = 0
    ...
    try:
        if supabase:
            pdd_result = supabase.table("pendapatan_diterima_dimuka").select("*").eq("user_email", user_email).eq("status", "dp_diterima").execute()
            pdd_belum_lunas = pdd_result.data or []
            total_dp_aktif = sum(float(item.get('jumlah_dp', 0)) for item in pdd_belum_lunas)
    except Exception as e:
        logger.error(f"Error ambil data DP: {str(e)}")

    # Handle POST request untuk pelunasan
    if request.method == "POST":
        try:
            dp_id = request.form.get("dp_id")
            tanggal_pelunasan = request.form.get("tanggal_pelunasan")
            metode_pembayaran = request.form.get("metode_pembayaran")
            keterangan_tambahan = request.form.get("keterangan_tambahan", "")
            
            if not dp_id or not tanggal_pelunasan or not metode_pembayaran:
                message = '<div class="message error">❌ Semua field harus diisi.</div>'
            else:
                # Ambil data DP
                dp_data = None
                for item in pdd_belum_lunas:
                    if str(item.get('id')) == dp_id:
                        dp_data = item
                        break
                
                if not dp_data:
                    message = '<div class="message error">❌ Data DP tidak ditemukan.</div>'
                else:
                    # Hitung jumlah
                    jumlah_dp = float(dp_data.get('jumlah_dp', 0))
                    total_harga_jual = float(dp_data.get('total_harga_jual', 0))
                    sisa_bayar = total_harga_jual - jumlah_dp
                    
                    # 🔹 UPDATE STATUS DP MENJADI PELUNASAN
                    update_data = {
                        "status": "pelunasan",
                        "tanggal_pelunasan": tanggal_pelunasan,
                        "metode_pelunasan": metode_pembayaran,
                        "keterangan_pelunasan": keterangan_tambahan,
                        "updated_at": datetime.now().isoformat()
                    }
                    
                    supabase.table("pendapatan_diterima_dimuka").update(update_data).eq("id", dp_id).execute()
                    
                    # 🔹 BUAT JURNAL UMUM UNTUK PELUNASAN - PAKAI 'LAINNYA'
                    # 1. Pendapatan Diterima Dimuka (2200) -> DEBIT (mengurangi kewajiban)
                    # 2. Piutang Usaha (1130) -> DEBIT (mencatat piutang)
                    # 3. Penjualan (4100) -> KREDIT (mencatat pendapatan)
                    
                    jurnal_pelunasan = [
                        # Jurnal 1: Pengakuan Pendapatan (mengurangi PDD)
                        {
                            "user_email": user_email,
                            "tanggal": tanggal_pelunasan,
                            "nama_akun": "Pendapatan Diterima Dimuka",  # Akun 2200
                            "debit": jumlah_dp,
                            "kredit": 0,
                            "keterangan": f"[PELUNASAN DP] Pelunasan DP dari {dp_data['nama_customer']}: {dp_data['keterangan']} | {keterangan_tambahan}",
                            "transaksi_type": "LAINNYA",  # DIUBAH
                            "referensi_id": dp_id,
                            "created_at": datetime.now().isoformat()
                        },
                        # Jurnal 2: Piutang dari sisa pembayaran
                        {
                            "user_email": user_email,
                            "tanggal": tanggal_pelunasan,
                            "nama_akun": "Piutang Usaha",  # Akun 1130
                            "debit": sisa_bayar if sisa_bayar > 0 else 0,
                            "kredit": 0,
                            "keterangan": f"[PELUNASAN DP] Piutang pelunasan dari {dp_data['nama_customer']} untuk {dp_data['jenis_ikan']}",
                            "transaksi_type": "LAINNYA",  # DIUBAH
                            "referensi_id": dp_id,
                            "created_at": datetime.now().isoformat()
                        },
                        # Jurnal 3: Pengakuan Pendapatan Penjualan
                        {
                            "user_email": user_email,
                            "tanggal": tanggal_pelunasan,
                            "nama_akun": "Penjualan",  # Akun 4100
                            "debit": 0,
                            "kredit": total_harga_jual,
                            "keterangan": f"[PELUNASAN DP] Penjualan {dp_data['jenis_ikan']} kepada {dp_data['nama_customer']} - Pelunasan DP",
                            "transaksi_type": "LAINNYA",  # DIUBAH
                            "referensi_id": dp_id,
                            "created_at": datetime.now().isoformat()
                        }
                    ...
                    # Insert jurnal ke database
                    for jurnal in jurnal_pelunasan:
                        supabase.table("jurnal_umum").insert(jurnal).execute()
                    
                    message = f'''
                    <div class="message success">
                        ✅ Pelunasan berhasil dicatat!
                        <div style="margin-top: 10px; background: #f0f8ff; padding: 10px; border-radius: 5px; border-left: 4px solid #4CAF50;">
                            <strong>📋 Detail Pelunasan:</strong><br>
                            • 👤 Customer: {dp_data['nama_customer']}<br>
                            • 🐟 Jenis Ikan: {dp_data['jenis_ikan']}<br>
                            • 💰 DP Awal: {format_currency(jumlah_dp)}<br>
                            • 🏷️ Total Harga: {format_currency(total_harga_jual)}<br>
                            • 🔄 Sisa Pelunasan: {format_currency(sisa_bayar)}<br>
                            • 📅 Tanggal Pelunasan: {tanggal_pelunasan}<br>
                            • 💳 Metode: {metode_pembayaran.title()}<br>
                            • 📝 <em>Jenis: Pelunasan DP (LAINNYA)</em><br>
                            <br>
                            <strong>📝 Jurnal yang dibuat:</strong><br>
                            1. Debit Pendapatan Diterima Dimuka: {format_currency(jumlah_dp)}<br>
                            2. Debit Piutang Usaha: {format_currency(sisa_bayar)}<br>
                            3. Kredit Penjualan: {format_currency(total_harga_jual)}
                        </div>
                    </div>
                    '''
            ...
            message = f'<div class="message error">❌ Error saat pelunasan: {str(e)}</div>'
            logger.error(f"Error pelunasan DP: {str(e)}")
            
    # Ambil riwayat pelunasan
    pdd_sudah_lunas = []
    try:
        if supabase:
            pdd_lunas_result = supabase.table("pendapatan_diterima_dimuka").select("*").eq("user_email", user_email).eq("status", "pelunasan").order("tanggal_pelunasan", desc=True).limit(10).execute()
            pdd_sudah_lunas = pdd_lunas_result.data or []
    except Exception as e:
        logger.error(f"Error ambil data pelunasan: {str(e)}")
    ...
    <html>
    <head>
        <title>Pelunasan Pendapatan Diterima Dimuka - PINKILANG</title>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <style>
            * {{
        ...
        <script>
            function updateCalculation() {{
                const dpId = document.getElementById('dp_id').value;
                if (!dpId) return;
                
                // Find the selected DP data
                const dpSelect = document.getElementById('dp_id');
                const selectedOption = dpSelect.options[dpSelect.selectedIndex];
                const jumlahDp = parseFloat(selectedOption.getAttribute('data-dp')) || 0;
                const totalHarga = parseFloat(selectedOption.getAttribute('data-total')) || 0;
                const customer = selectedOption.getAttribute('data-customer') || '';
                const jenisIkan = selectedOption.getAttribute('data-jenis') || '';
                
                if (jumlahDp && totalHarga) {{
                    const sisaBayar = totalHarga - jumlahDp;
                    
                    // Update calculation display
                    document.getElementById('calc_dp').textContent = formatCurrency(jumlahDp);
                    document.getElementById('calc_total').textContent = formatCurrency(totalHarga);
                    document.getElementById('calc_sisa').textContent = formatCurrency(sisaBayar);
                    
                    // Update jurnal preview
                    document.getElementById('jurnal_dp').textContent = formatCurrency(jumlahDp);
                    document.getElementById('jurnal_piutang').textContent = formatCurrency(sisaBayar);
                    document.getElementById('jurnal_penjualan').textContent = formatCurrency(totalHarga);
                    
                    // Update info
                    document.getElementById('dp_info').textContent = `${{customer}} - ${{jenisIkan}}`;
                }}
            }}
            ...
            document.addEventListener('DOMContentLoaded', function() {{
                ...
                document.getElementById('tanggal_pelunasan').valueAsDate = new Date();
            }});
        </script>
    </head>
    <body>
        <div class="container">
            <div class="header">
                <a href="/dashboard" class="back-btn">← Kembali ke Dashboard</a>
                <h1>💰 Pelunasan Pendapatan Diterima Dimuka</h1>
                <div class="subtitle">
                    Login sebagai: <strong>{user_email}</strong> | 
                    Sistem: <strong>Jurnal Akuntansi Otomatis</strong>
                </div>
            </div>
            ...
            <div class="content">
                ...
                <div class="stats-grid">
                    <div class="stat-card">
                        <div class="stat-number">{format_currency(total_dp_aktif)}</div>
                        <div class="stat-label">Total DP Aktif</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-number">{len(pdd_belum_lunas)}</div>
                        <div class="stat-label">Transaksi Belum Lunas</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-number">{len(pdd_sudah_lunas)}</div>
                        <div class="stat-label">Transaksi Sudah Lunas</div>
                    </div>
                </div>
                ...
                <div class="section">
                    <h2 class="section-title">
                        <i>💳</i> Form Pelunasan DP
                    </h2>
                    
                    <form method="POST">
                        <div class="form-grid">
                            <div class="form-group">
                                <label for="dp_id">📋 Pilih Transaksi DP:</label>
                                <select id="dp_id" name="dp_id" required onchange="updateCalculation()">
                                    <option value="">-- Pilih Transaksi DP --</option>
                                    {"".join([f'''
                                    <option value="{item['id']}" 
                                        data-dp="{item.get('jumlah_dp', 0)}"
                                        data-total="{item.get('total_harga_jual', 0)}"
                                        data-customer="{item.get('nama_customer', '')}"
                                        data-jenis="{item.get('jenis_ikan', '')}">
                                        {item['tanggal']} - {item['nama_customer']} - {format_currency(item.get('jumlah_dp', 0))} ({item.get('dp_persen', 0)}% dari {format_currency(item.get('total_harga_jual', 0))})
                                    </option>
                                    ''' for item in pdd_belum_lunas])}
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" id="tanggal_pelunasan" name="tanggal_pelunasan" required>
                            </div>
                            
                            <div class="form-group">
                                <label for="metode_pembayaran">💳 Metode Pelunasan:</label>
                                <select id="metode_pembayaran" name="metode_pembayaran" required>
                                    <option value="tunai">Tunai (Kas)</option>
                                    <option value="transfer">Transfer (Bank)</option>
                            ...
                            <div class="form-group full-width">
                                <label for="keterangan_tambahan">📝 Keterangan Pelunasan:</label>
                                <textarea id="keterangan_tambahan" name="keterangan_tambahan" rows="3" placeholder="Contoh: Pelunasan penjualan ikan, diterima tunai..."></textarea>
                            </div>
                        </div>
                        
                        <!-- PREVIEW PERHITUNGAN -->
                        <div class="calculation-box">
                            <h3 style="color: #0D47A1; margin-bottom: 15px;">📊 Preview Perhitungan</h3>
                            <div id="dp_info" style="background: #e3f2fd; padding: 10px; border-radius: 6px; margin-bottom: 15px; color: #0D47A1; font-weight: 500;">
                                Pilih transaksi DP untuk melihat perhitungan
                            </div>
                            <div class="calc-row">
                                <span class="calc-label">Jumlah DP Diterima:</span>
                                <span class="calc-value" id="calc_dp">Rp 0</span>
                            </div>
                            <div class="calc-row">
                                <span class="calc-label">Total Harga Jual:</span>
                                <span class="calc-value" id="calc_total">Rp 0</span>
                            </div>
                            <div class="calc-row">
                                <span class="calc-label"><strong>Sisa Pelunasan:</strong></span>
                                <span class="calc-value" id="calc_sisa"><strong>Rp 0</strong></span>
                            </div>
                        </div>
                        
                        <!-- PREVIEW JURNAL -->
                        <div class="jurnal-preview">
                            <h3 style="color: #2E7D32; margin-bottom: 15px;">📝 Jurnal yang akan dibuat:</h3>
                            <div style="margin-bottom: 15px; color: #666; font-size: 13px;">
                                Ketika pelunasan dilakukan, sistem akan membuat jurnal akuntansi berikut:
                            </div>
                            <div style="background: white; padding: 15px; border-radius: 6px; border: 1px solid #e3f2fd;">
                                <div style="display: grid; grid-template-columns: 100px 1fr 120px 120px; gap: 10px; margin-bottom: 10px; font-weight: bold; color: #0D47A1; padding: 8px; background: #f8fdff; border-radius: 4px;">
                                    <div>No.</div>
                                    <div>Akun</div>
                                    <div>Debit</div>
                                    <div>Kredit</div>
                                </div>
                                <div style="display: grid; grid-template-columns: 100px 1fr 120px 120px; gap: 10px; margin: 8px 0; padding: 8px; background: #f8fdff; border-radius: 4px; border-left: 4px solid #2196F3;">
                                    <div>1.</div>
                                    <div>Pendapatan Diterima Dimuka (2200)</div>
                                    <div class="number" id="jurnal_dp">Rp 0</div>
                                    <div>-</div>
                                </div>
                                <div style="display: grid; grid-template-columns: 100px 1fr 120px 120px; gap: 10px; margin: 8px 0; padding: 8px; background: #f8fdff; border-radius: 4px; border-left: 4px solid #2196F3;">
                                    <div>2.</div>
                                    <div>Piutang Usaha (1130)</div>
                                    <div class="number" id="jurnal_piutang">Rp 0</div>
                                    <div>-</div>
                                </div>
                                <div style="display: grid; grid-template-columns: 100px 1fr 120px 120px; gap: 10px; margin: 8px 0; padding: 8px; background: #f8fdff; border-radius: 4px; border-left: 4px solid #4CAF50;">
                                    <div>3.</div>
                                    <div>Penjualan (4100)</div>
                                    <div>-</div>
                                    <div class="number" id="jurnal_penjualan">Rp 0</div>
                                </div>
                            </div>
                        </div>
                        
                        <button type="submit" class="btn btn-success" style="width: 100%;">
                            ✅ Proses Pelunasan & Buat Jurnal
                        </button>
                    </form>
                </div>
                ...
                <div class="section">
                    <h2 class="section-title">
                        <i>⏳</i> DP Belum Dilunasi ({len(pdd_belum_lunas)} transaksi)
                    </h2>
                    <div class="table-container">
                        <table>
                            <thead>
                            ...
                            <tbody>
                                {generate_pdd_rows_2(pdd_belum_lunas) if pdd_belum_lunas else '''
                                <tr>
                                    <td colspan="8" style="text-align: center; padding: 30px; color: #666;">
                                        Tidak ada DP yang belum dilunasi
                                    </td>
                                </tr>
                                '''}
                            </tbody>
                ...
                <div class="section">
                    <h2 class="section-title">
                        <i>✅</i> Riwayat Pelunasan ({len(pdd_sudah_lunas)} transaksi)
                    </h2>
                    <div class="table-container">
                        <table>
                            <thead>
                            ...
                            <tbody>
                                {generate_pelunasan_rows(pdd_sudah_lunas) if pdd_sudah_lunas else '''
                                <tr>
                                    <td colspan="8" style="text-align: center; padding: 30px; color: #666;">
                ...
                <!-- ACTION BUTTONS -->
                <div class="action-buttons">
                    <a href="/pendapatan-diterima-dimuka" class="btn">➕ Input DP Baru</a>
                    <a href="/jurnal-umum" class="btn btn-secondary">📝 Lihat Jurnal Umum</a>
                    <a href="/neraca-lajur" class="btn">📊 Lihat Neraca Lajur</a>
                    <a href="/dashboard" class="btn">🏠 Dashboard</a>
                </div>
...
def generate_pdd_rows(pdd_data):
    """Generate HTML rows untuk tabel Pendapatan Diterima Dimuka (route pendapatan)"""
    rows = ""
...
# ============================================================
# 🔹 FUNGSI BANTU: Pelunasan Pendapatan Diterima Dimuka
# ============================================================

def generate_pelunasan_rows(pdd_data):
    """Generate HTML rows untuk riwayat pelunasan"""
    rows = ""
    for item in pdd_data:
        jumlah_dp = float(item.get('jumlah_dp', 0))
        total_harga = float(item.get('total_harga_jual', 0))
        sisa_bayar = total_harga - jumlah_dp
        
        rows += f"""
        <tr>
            <td>{item.get('tanggal', '-')}</td>
            <td>{item.get('tanggal_pelunasan', '-')}</td>
            <td>{item.get('nama_customer', '-')}</td>
            <td class="number">{format_currency(total_harga)}</td>
            <td class="number">{format_currency(jumlah_dp)}</td>
            <td class="number">{format_currency(sisa_bayar)}</td>
            <td>{item.get('metode_pelunasan', '-').title()}</td>
            <td><span class="status-badge badge-lunas">✅ LUNAS</span></td>
        </tr>
        """
    return rows


def generate_pdd_rows_2(pdd_data):
    """Generate HTML rows untuk DP belum lunas (route pelunasan)"""
    rows = ""
    for item in pdd_data:
        jumlah_dp = float(item.get('jumlah_dp', 0))
        total_harga = float(item.get('total_harga_jual', 0))
        ...
        <tr>
            <td>{item.get('tanggal', '-')}</td>
            <td>{item.get('nama_customer', '-')}</td>
            <td>{item.get('jenis_ikan', '-')}</td>
            <td class="number">{item.get('jumlah_ekor', 0)} ekor</td>
            <td class="number">{format_currency(total_harga)}</td>
            <td>{item.get('dp_persen', 0)}%</td>
            <td class="number">{format_currency(jumlah_dp)}</td>
            <td class="number">{format_currency(sisa_bayar)}</td>
        </tr>
        """
            ...
            <div class="container">
                ...
                <div class="content">
                        ...
                        <table class="worksheet-table">
                            ...
                            <tfoot>
                                ...
                                <tr style="background: linear-gradient(135deg, #fff8e1 0%, #fff5e6 100%); border-left: 5px solid #ffcc00;">
                                    <td><strong>⚖️ FINAL BALANCE CHECK</strong></td>
                                    <td colspan="8"></td>
                    ...
                    <div class="action-buttons">
                        ...
                        <button onclick="window.print()" class="btn" style="background: linear-gradient(135deg, #ff99cc 0%, #ff66b2 100%);">🖨️ Cetak</button>
...
# ============================================================
# 🔹 ROUTE: Aset (Menu Utama) - UPDATED dengan REAL DATA
# ============================================================
@app.route("/aset")
def aset():
    if not session.get('logged_in'):
